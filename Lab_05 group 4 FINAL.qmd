...
It can also be classified depending of the `race` variable and showing the most prevalent races at the top by wrapping `Race` inside `fct_rev(fct_infreq(()))`:
...
The most important aspect of this project would be to map which peptides that have the highest hit rate for CD3R (the T-cell receptor). A quantification of the number of hits per peptide would allow us to ascertain which peptides incite the highest expansion of T-cell receptors as a proxy for specific T cells, which would then show which peptides are the most immunogenic in the SARS-CoV2 peptidome. 
The peptide-hits data are actually available as the file `peptide-hits-ci.csv` but strangely it provides the number of hits per peptide pool and not per individual peptide. 
